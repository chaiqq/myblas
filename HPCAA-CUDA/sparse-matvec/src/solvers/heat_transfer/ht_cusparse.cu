...
void PoissonSolver::poisson_cusparse() {
    ...
    for (int counter = 0; ((err > settings.epsilon) && (settings.numIterations > counter)); ++counter) {
        ...
        // TODO: H4.1
        CUSPARSE_CHECK(cusparseShybmv(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha,
                                      descr, hybridMatrix, devX, &beta, devY));

        // computes the Euclidean norm of the vector devY using cublas
        // TODO: H4.1 similar to T4.1
        CUBLAS_CHECK(cublasSnrm2(cublasHandle, N, devY, 1, &err));

        alpha = dt / (dx * dx) * settings.conductivity;
        // TODO: H4.1 similar to T4.1
        CUBLAS_CHECK(cublasSaxpy(cublasHandle, N, &alpha, devY, 1, devX, 1));
